...
resumeRouter.post("/parse", requireAuth, upload.single("file"), async (req, res, next) => {
  try {
    ...
    // pdf-parse can be CJS or ESM depending on tooling; support both.
    const parseFn =
      (pdfParse as any)?.default ??
      (pdfParse as any);
    if (typeof parseFn !== "function") throw new ApiError(500, "Resume parser not available");

    const parsed = await (parseFn(req.file.buffer) as Promise<{ text?: string }>);
    const text = (parsed.text ?? "").slice(0, 200_000);
